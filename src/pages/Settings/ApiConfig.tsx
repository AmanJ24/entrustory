      ...
      <div className="flex flex-col w-full max-w-[1400px] mx-auto gap-8 pb-20">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Link 
            ...
            className="flex items-center justify-center rounded-lg h-11 px-5 bg-slate-800 hover:bg-slate-700 text-white gap-2 text-sm font-bold transition-all border border-slate-700 shadow-lg"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#111722] border border-slate-800 rounded-xl p-5 flex items-center gap-4 shadow-lg">
            <div className="w-12 h-12 rounded-full bg-cyan-500/10 flex items-center justify-center text-cyan-400 border border-cyan-500/20"><Key size={24} /></div>
            <div><p className="text-slate-400 text-sm font-medium">Active API Keys</p><h3 className="text-2xl font-bold text-white">{keys.length}</h3></div>
          </div>
          <div className="bg-[#111722] border border-slate-800 rounded-xl p-5 flex items-center gap-4 shadow-lg">
            ...
            <div><p className="text-slate-400 text-sm font-medium">CLI/API Requests</p><h3 className="text-2xl font-bold text-white">{apiRequestsCount}</h3></div>
          </div>
          <div className="bg-[#111722] border border-slate-800 rounded-xl p-5 flex items-center gap-4 shadow-lg">
            ...
            <div><p className="text-slate-400 text-sm font-medium">DB Query Latency</p><h3 className="text-2xl font-bold text-white">{realLatency > 0 ? `${realLatency}ms` : '--'}</h3></div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-8 lg:col-span-7">
                  ...
                  keys.map((k) => (
                    <div key={k.id} className="bg-[#111722] border border-slate-800 rounded-xl p-5 hover:border-slate-700 transition-all shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-full bg-cyan-500/10 text-cyan-400 flex items-center justify-center border border-cyan-500/20 shrink-0 mt-1"><Key size={18} /></div>
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <code className="text-slate-400 bg-[#0B1120] border border-slate-800 px-2 py-1 rounded text-sm font-mono">{maskKey(k.key_value)}</code>
            ...
            <section className="flex flex-col gap-4 mt-4">
              ...
              <div className="bg-[#111722] border border-slate-800 rounded-xl p-5 shadow-lg flex flex-col sm:flex-row justify-between items-center gap-4 hover:border-slate-700 transition-colors">
                ...
                <button onClick={handleTestPing} disabled={pinging} className={`px-4 py-2 text-xs font-bold rounded-lg flex items-center gap-2 transition-all border ${pingSuccess ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-slate-800 text-slate-300 hover:bg-slate-700 border-slate-700'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#111722] border border-slate-800 rounded-xl shadow-2xl w-full max-w-md p-6 relative">
            ...
            <form onSubmit={handleCreateKey} className="space-y-5 mt-4">
              ...
              <button type="submit" className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 rounded-lg shadow-lg mt-2 transition-all">Generate Key</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md px-4">
          <div className="bg-[#111722] border border-cyan-500/50 rounded-xl shadow-[0_0_50px_rgba(6,182,212,0.2)] w-full max-w-lg p-8 relative text-center">
            ...
            <div className="bg-[#0B1120] border border-slate-700 rounded-lg p-4 mb-6 flex items-center justify-between">
              ...
              <button onClick={() => copyToClipboard(revealedKey, 'reveal')} className="ml-4 p-2 bg-slate-800 hover:bg-slate-700 rounded text-white">
                {copiedKey === 'reveal' ? <CheckCircle size={18} className="text-emerald-400" /> : <Copy size={18} />}
              </button>
            </div>
            <button onClick={() => { setRevealedKey(null); setCopiedKey(null); }} className="w-full bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white font-bold py-3 rounded-lg transition-all">I have stored it safely</button>
          </div>
        </div>
      )}
    </div>
