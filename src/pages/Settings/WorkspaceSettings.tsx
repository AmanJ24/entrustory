    ...
    <div className="bg-[#0B1120] font-['Inter'] text-slate-100 min-h-full flex relative z-0">
      ...
      <aside className="w-64 border-r border-slate-800 bg-[#0B1120] shrink-0 pt-8 px-4 flex flex-col h-[calc(100vh-104px)] sticky top-0">
        ...
        <nav className="flex flex-col gap-1">
          <button onClick={() => setActiveTab('general')} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'general' ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}><Building2 size={18} /> General</button>
          <button onClick={() => setActiveTab('security')} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'security' ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}><ShieldCheck size={18} /> Security & Access</button>
          <button onClick={() => setActiveTab('crypto')} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'crypto' ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}><Key size={18} /> Cryptography</button>
          <button onClick={() => setActiveTab('notifications')} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'notifications' ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}><Bell size={18} /> Integrations</button>
          <button onClick={() => setActiveTab('compliance')} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'compliance' ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200'}`}><Globe size={18} /> Data & Compliance</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-8 lg:p-12 overflow-y-auto pb-32">
        <div className="max-w-4xl">
            ...
            <div className="space-y-10 animate-in fade-in duration-300">
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Workspace ID (Read Only)</label>
              ...
              <section className="space-y-4 pt-4">
                ...
                <div className="border border-red-500/30 rounded-xl overflow-hidden">
                  <div className="p-4 bg-red-500/5 flex justify-between items-center border-b border-red-500/20">
                    ...
                    <button onClick={handleTransferOwnership} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-sm font-bold rounded-lg border border-slate-700 transition-colors">Transfer</button>
                  </div>
                  <div className="p-4 bg-red-500/5 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Permanently delete all cryptographic records and vaults.</p>
                    </div>
              ...
              <section className="space-y-6">
                ...
                <div className="p-4 bg-[#111722] border border-slate-800 rounded-xl">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={handleConnectSSO} className="px-4 py-2 border border-slate-700 hover:bg-slate-800 text-white rounded-lg text-sm font-bold transition-colors">Configure SSO</button>
                  </div>
                </div>
              </section>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-3 p-3 border border-cyan-500 bg-cyan-500/5 rounded-lg cursor-pointer">
                        <input type="radio" checked={hashAlgo === 'sha256'} onChange={() => setHashAlgo('sha256')} className="text-cyan-500" />
                        <div><p className="text-sm font-bold text-cyan-400">SHA-256</p></div>
                      </label>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-10 animate-in fade-in duration-300">
              <section className="space-y-4">
                <h3 className="text-lg font-bold text-white border-b border-slate-800 pb-2">Third-Party Connections</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                  <div className="border border-slate-800 bg-[#111722] rounded-xl p-5 hover:border-slate-600 transition-all flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-slate-800 text-slate-400 px-2 py-1 rounded">Disabled</span>
                      </div>
                      <h4 className="font-bold text-white mb-1 text-sm">Amazon S3 Backup</h4>
                      <p className="text-xs text-slate-400 mb-4">Automate nightly database and vault backups to your AWS bucket.</p>
                    </div>
                    {/* FIXED: S3 Button Action */}
                    <button onClick={handleConnectS3} className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-lg transition-colors border border-slate-700">Connect S3</button>
                  </div>
                </div>
              </section>
            </div>
          ...
          {activeTab === 'compliance' && (
            <div className="py-20 text-center border-2 border-dashed border-slate-800 rounded-xl bg-[#111722]/50">
              ...
              <p className="text-slate-400">Detailed compliance reporting requires an Enterprise support ticket to provision.</p>
            </div>
          )}
