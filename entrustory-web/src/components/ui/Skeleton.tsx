  ...
  return (
    <>
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className={`animate-pulse bg-surface-variant rounded-lg ${className}`}
        />
      ))}
    </>
  );
  ...
  return (
    <>
      {Array.from({ length: rows }).map((_, rowIdx) => (
        <tr key={rowIdx} className="border-b border-surface-variant/50">
          {Array.from({ length: columns }).map((_, colIdx) => (
            <td key={colIdx} className="px-6 py-4">
              <div className="animate-pulse bg-surface-variant rounded h-4 w-full" style={{ maxWidth: `${60 + Math.random() * 40}%` }} />
            </td>
          ))}
        </tr>
      ))}
...
export const MetricCardSkeleton = () => {
  return (
    <div className="bg-surface-container-low border border-surface-variant rounded-xl p-6 animate-pulse">
      <div className="flex items-center gap-4">
        <div className="w-12 h-12 rounded-full bg-surface-variant" />
        <div className="flex-1 space-y-2">
          <div className="h-3 bg-surface-variant rounded w-24" />
          <div className="h-6 bg-surface-variant rounded w-16" />
        </div>
      </div>
    </div>
  );
